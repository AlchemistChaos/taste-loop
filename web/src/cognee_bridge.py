...
async def cmd_distill(args):
    ...
    try:
        ...
        if docs:
            lessons = [{"statement": d.strip()} for d in docs if d and d.strip()]
            if lessons:
    ...
    statement = texts[0].strip()
    # Persist into the long-term lessons dataset so recall_lessons can find it later.
    with contextlib.suppress(Exception):
        await cognee.add(statement, dataset_name=LONG_TERM_DATASET)
        await cognee.cognify(datasets=[LONG_TERM_DATASET])
    return {"lessonsAccepted": [{"statement": statement}], "via": "fallback"}
